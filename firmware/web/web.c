...
extern void screen_draw_httpd_string(char *string_ptr);
...
THD_FUNCTION(http_server, p) {
  ...
  netconn_bind(conn, IP4_ADDR_ANY, WEB_THREAD_PORT);
  ...
  screen_draw_httpd_string("Starting loop");
  while (true)
  {
    screen_draw_httpd_string("In loop");
    err = netconn_accept(conn, &newconn);
    screen_draw_httpd_string("accepted connection");
